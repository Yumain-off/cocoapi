...
class COCOeval:
    ...
    def evaluateImg(self, imgId, catId, aRng, maxDet):
        ...
        for g in gt:
            if g['ignore'] or (g['area'] < aRng[0] or g['area'] > aRng[1]):
                ...
                g['_ignore'] = 0
                if str(catId) not in self.label_info_on_range.keys():
                    self.label_info_on_range[str(catId)] = []
                if len(self.label_info_on_range[str(catId)]) == 0:
                    ...
                    try:
                        ...
                        cur_range_index = self.label_info_on_range[str(catId)].index(cur_range)
                    except IndexError:
                        self.label_info_on_range[str(catId)].append(
                            {'aRng': aRng, 'aRng_label': self.params.areaRngLbl[self.params.areaRng.index(aRng)],
                             'gt_count': 0})
    ...
    def summarize(self, get_eval_stats=False, verbose_value_per_category=False):
        ...
        def _summarize(ap=1, iouThr=None, areaRng='all', maxDets=100, verbose_value_per_category=False):
            p = self.params
            ...
            if len(s[s > -1]) == 0:
                ...
                avg_ap = 0.0
                if ap == 1 and verbose_value_per_category:
                    print('AP for a treshold of {} and an area of {}'.format(iouThr, areaRng))
                    for i, catId in enumerate(p.catIds):
                        tweak_prec = s[:, :, i, :]
                        tweak_prec[tweak_prec < 0.] = 0.
                        label_ap = np.mean(tweak_prec)
                        try:
        ...
        def _summarizeDets(verbose_value_per_category=False):
            stats = np.zeros((12,))
            stats[0] = _summarize(1,
                                  verbose_value_per_category=verbose_value_per_category)
        ...
        def _summarizeKps(verbose_value_per_category=False):
            stats = np.zeros((10,))
